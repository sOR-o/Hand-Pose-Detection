        ...
        if results.multi_hand_landmarks:
            for landmarks, hand in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                if hand.classification[0].label == "Left":
                    ...
                    # Draw a circle at the closer_point coordinate
                    circle_radius = 10  # Adjust the radius as needed
                    circle_color = (128, 0, 128)  # purple color
                    cv2.circle(image, tuple(closer_point[:2].astype(int)), circle_radius, circle_color, -1)
